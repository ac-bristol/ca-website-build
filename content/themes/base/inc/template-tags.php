...
function base_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Posts navigation', 'base' ); ?></h1>
		<div class="nav-links">
...
function base_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<p class="screen-reader-text"><?php _e( 'Post navigation', 'base' ); ?></p>
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span>&nbsp;Previous', 'Previous post link', 'base' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( 'Next&nbsp;<span class="meta-nav">&rarr;</span>', 'Next post link',     'base' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function base_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'base' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'base' ),
...
function base_categorized_blog() {
	...
	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so base_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so base_categorized_blog should return false.
		return false;
	}
}
